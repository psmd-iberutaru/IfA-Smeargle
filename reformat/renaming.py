...
def detector_renameing(data_directory, detector_name):
    ...
    detector_string_list : list
        This is the list of the state on if the object is garbage or not.
    detector_raw_list : list 
        This is the raw, unformatting values that was formatted into the
        string version.
    """
...
def garbage_renaming(data_directory, begin_garbage=0):
    """ Creates file tags according to their status as garbage. If a file is 
    garbage, it should generally not be used.
    ...
    begin_garbage : int (optional)
        ...
        (i.e. the number of garbage files).
    ...
    garbage_raw_list = [True if index < begin_garbage else False for index in range(n_files)]
    # And their formatted string value.
    garbage_string_list = ['garb;Y' if garbdex else 'garb;N' for garbdex in garbage_raw_list]
...
def number_renaming(data_directory, begin_garbage=0):
    ...
    # such.
    garbage_names = core.io.get_fits_filenames(data_directory=data_directory)[:begin_garbage]
    ...
    original_names = core.io.get_fits_filenames(data_directory=data_directory)[begin_garbage:]
...
def set_renaming(data_directory, set_length, begin_garbage=0):
    """ Creates file tags according to their set number, as determined by the
    ...
    set_raw_list = garbage_numbers + [(numdex-1)//set_length + 1 for numdex in file_numbers]
...
def voltage_pattern_renaming(data_directory, voltage_pattern, 
    ...
    voltage_raw_list : dictionary
        This is the raw, unformatting values that was formatted into the
        string version. The voltage values and trends are seperated (as 
        `value` and `trend`).
    ...
    # Formatting the voltage trends by the increase or decrease of the 
    # voltages.
    ...
    # Extracting the raw values from the formatted values and converting.
    raw_voltages = [float(core.strformat.purge_substrings(string=voltdex, 
                                                    substrings=(list(string.ascii_uppercase) 
                                                                + list(string.ascii_lowercase) 
                                                                + [':',';'])))
                    for voltdex in (garbage_string_list + voltage_string_list)]
    raw_trends = [str(core.strformat.purge_substrings(string=voltdex, 
                                                      substrings=(list(string.digits) 
                                                                  + ['.',';','detBias','V'])))
                    for voltdex in (garbage_string_list + voltage_string_list)]
...
def script_detector_renameing(config):
    """ The scripting version of `detector_renameing`. This function applies the
    ...
    data_directory = core.config.extract_configuration(config_object=config, 
                                                       keys=['data_directory'])
    detector_name = core.config.extract_configuration(config_object=config, 
                                                      keys=['renaming','detector_name'])

    # Obtain the labels.
    labels, raw = detector_renameing(data_directory=data_directory, detector_name=detector_name)
    
    # Add to all file headers. Assume that the order has not changed between 
    # renaming steps.
    core.error.ifas_info("Adding the detector name `{detect_name}` under the `DETECTOR` card "
                         "in the headers of the fits files in {data_dir}."
                         .format(detect_name=detector_name,data_dir=data_directory))
    fits_files = core.io.get_fits_filenames(data_directory=data_directory)
    for (filedex, headerdex) in zip(fits_files, raw):
        __ = core.io.append_astropy_header_card(file_name=filedex, 
                                                header_cards={'DETECTOR':headerdex})

    # Finally rename the files based on parallel appending. Glob provides the
    # directory.
    core.error.ifas_info("Appending the detector name to the end of the files in {data_dir}."
                         .format(data_dir=data_directory))
    core.io.rename_by_parallel_append(file_names=fits_files, appending_names=labels, 
                                      directory=None)

    return None

def script_garbage_renaming(config):
    """ The scripting version of `garbage_renaming`. This function applies the
    rename to the entire directory. It also adds the tags to the header file
    of each fits.
    ...
    data_directory = core.config.extract_configuration(config_object=config, 
                                                       keys=['data_directory'])
    begin_garbage = core.config.extract_configuration(config_object=config, 
                                                      keys=['renaming','begin_garbage'])

    # Obtain the labels.
    labels, raw = garbage_renaming(data_directory=data_directory, begin_garbage=begin_garbage)
    
    # Add to all file headers. Assume that the order has not changed between 
    # renaming steps.
    core.error.ifas_info("Adding the garbage status under the `GARBAGE` card "
                         "in the headers of the fits files in {data_dir}."
                         .format(data_dir=data_directory))
    fits_files = core.io.get_fits_filenames(data_directory=data_directory)
    for (filedex, headerdex) in zip(fits_files, raw):
        __ = core.io.append_astropy_header_card(file_name=filedex, 
                                                header_cards={'GARBAGE':headerdex})

    # Finally rename the files based on parallel appending. Glob provides the
    # directory.
    core.error.ifas_info("Appending the garbage status to the end of the files in {data_dir}."
                         .format(data_dir=data_directory))
    core.io.rename_by_parallel_append(file_names=fits_files, appending_names=labels, 
                                      directory=None)
...
def script_number_renaming(config):
    """ The scripting version of `number_renaming`. This function applies the
    rename to the entire directory. It also adds the tags to the header file
    of each fits.
    ...
    # Obtain the labels.
    labels, raw = number_renaming(data_directory=data_directory, begin_garbage=begin_garbage)
    
    # Add to all file headers. Assume that the order has not changed between 
    # renaming steps.
    core.error.ifas_info("Adding the file number under the `NUMBER` card "
                         "in the headers of the fits files in {data_dir} based on the "
                         "file order."
                         .format(data_dir=data_directory))
    fits_files = core.io.get_fits_filenames(data_directory=data_directory)
    for (filedex, headerdex) in zip(fits_files, raw):
        __ = core.io.append_astropy_header_card(file_name=filedex, 
                                                header_cards={'NUMBER':headerdex})

    # Finally rename the files based on parallel appending. Glob provides the
    # directory.
    core.error.ifas_info("Appending the file number to the end of the files in {data_dir}."
                         .format(data_dir=data_directory))
    core.io.rename_by_parallel_append(file_names=fits_files, appending_names=labels, 
                                      directory=None)
...
def script_set_renaming(config):
    """ The scripting version of `set_renaming`. This function applies the
    rename to the entire directory. It also adds the tags to the header file
    of each fits.
    ...
    data_directory = core.config.extract_configuration(config_object=config, 
                                                       keys=['data_directory'])
    set_length = core.config.extract_configuration(config_object=config,
                                                   keys=['renaming','set_length'])
    begin_garbage = core.config.extract_configuration(config_object=config, 
                                                      keys=['renaming','begin_garbage'])

    # Obtain the labels.
    labels, raw = set_renaming(data_directory=data_directory, set_length=set_length, 
    ...
    for (filedex, headerdex) in zip(fits_files, raw):
        __ = core.io.append_astropy_header_card(file_name=filedex, 
                                                header_cards={'SET_NUM':headerdex})

    # Finally rename the files based on parallel appending. Glob provides the
    # directory.
    core.error.ifas_info("Appending the set number to the end of the files in {data_dir}."
                         .format(data_dir=data_directory))
    core.io.rename_by_parallel_append(file_names=fits_files, appending_names=labels, 
                                      directory=None)
...
def script_voltage_pattern_renameing(config):
    """ The scripting version of `voltage_pattern_renaming`. This function applies the
    rename to the entire directory. It also adds the tags to the header file
    of each fits.
    ...
    data_directory = core.config.extract_configuration(config_object=config, 
                                                       keys=['data_directory'])
    voltage_pattern = core.config.extract_configuration(config_object=config, 
                                                      keys=['renaming','voltage_pattern'])
    begin_garbage = core.config.extract_configuration(config_object=config, 
                                                      keys=['renaming','begin_garbage'])

    # Obtain the labels. This one is a little special with its output; the
    # raw contains two sections.
    ...
    # Add to all file headers. Assume that the order has not changed between 
    # renaming steps.
    core.error.ifas_info("Adding the bias voltage under the `VOLTAGE` card and the ramp slope "
                         "under the `RAMPSLPE` card in the headers of the fits files in "
                         "{data_dir} based on the file order."
                         .format(data_dir=data_directory))
    fits_files = core.io.get_fits_filenames(data_directory=data_directory)
    for (filedex, voltdex, trenddex) in zip(fits_files, raw_volt_value, raw_volt_trend):
        __ = core.io.append_astropy_header_card(file_name=filedex, 
                                             header_cards={'VOLTAGE':voltdex,
                                                           'RAMPSLPE':trenddex})

    # Finally rename the files based on parallel appending. Glob provides the
    # directory.
    core.error.ifas_info("Appending the bias voltage number and slope to the end of the files "
                         "in {data_dir}."
                         .format(data_dir=data_directory))
    core.io.rename_by_parallel_append(file_names=fits_files, appending_names=labels, 
                                      directory=None)
