...
def sanitize_file_size(data_directory, method='largest', delete=False, exact_size=None):
    ...
    exact_size : int (optional)
        The exact size a proper file size should be (unit is in bytes). Only 
        applied if the method used is `exact`
    ...
    if (method == 'largest'):
        ...
        data_files = core.io.get_fits_filenames(data_directory=data_directory, recursive=True)
        ...
        if (exact_size is None):
            raise core.error.InputError("The method of file size sanitization is exact size. A "
    ...
    # input the correct method.
        ...
        raise core.error.InputError("The method provided does not exist or it cannot be "
                                    "understood in the form provided.")
    ...
    def find_improper_file_sizes(data_directory, proper_file_size):
        ...
        for filedex in data_files:
            ...
            if (os.path.getsize(filedex) == proper_file_size):
                ...
                core.error.ifas_info(("The fits file {bad_fits} did not pass file size sanitation."
                                   .format(bad_fits=str(filedex))), console_print=False)

        return bad_data_files

    # Find the bad files.
    bad_data_files = find_improper_file_sizes(data_directory=data_directory, 
                                         proper_file_size=proper_file_size)

    # Do basic checks before deleting the files to warn of inconsistancies.
    if (len(data_files) == len(bad_data_files)):
        core.error.ifas_warning(core.error.DataWarning,
                          ("All files within the given directory have been marked as bad by "
                           "`sanitize_file_size`. Please double check parameters."))
    if (proper_file_size == 0):
        core.error.ifas_warning(core.error.InputWarning,
                          ("The proper file size for `sanitize_file_size` is 0 bytes."))
    if (len(bad_data_files) == 0):
        ...
        core.error.ifas_log_warning("The following files are flagged for sanitization by "
                                    "`sanitize_file_size`: \n {flagged_files} "
                                    .format(flagged_files='\n'.join(
                                        [str(filedex) for filedex in bad_data_files])))
...
def script_sanitize_file_size(config):
    """ The scripting version of `sanitize_file_size`. This function sanitizes
    the documents as prescribed by the configuration and the inner sfunction.
    ...
    data_directory = core.config.extract_configuration(config_object=config, 
                                                       keys=['data_directory'])
    method = core.config.extract_configuration(config_object=config,
                                               keys=['sanitization', 'filesize', 'method'])
    exact_size = core.config.extract_configuration(config_object=config, 
                                                   keys=['sanitization', 'filesize', 'exact_size'])
    delete = core.config.extract_configuration(config_object=config, 
                                               keys=['sanitization', 'delete'])

    # Execture the inner function.
