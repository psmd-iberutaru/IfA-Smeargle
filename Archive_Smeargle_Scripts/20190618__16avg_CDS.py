...
fits_file_list = glob.glob('./**/Mk20_M12145-40/**/set4/*16avg_CDS.fits',recursive=True)
...
mean_array = [183,183,15,15,21,21,40,40,86,86,125,125]
counter = 0
# List of figures.\n",
for filedex,meandex in zip(fits_file_list[counter:],mean_array):
    counter += 1
    ...
    # Hot filtering
    masks = ifa.echo.echo_270_minimum_cut(data, meandex - 200,previous_mask=masks)
    masks = ifa.echo.echo_271_maximum_cut(data, meandex + 50, previous_mask=masks)
    ...
    bin_list = np.arange(np_ma.min(final_data) - 1,np_ma.max(final_data) + 1,1)
