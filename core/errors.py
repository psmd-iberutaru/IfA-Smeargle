...
class MagicError(Ifas_Exception):
    """
    This error is used when any routine would utilize magic/hard-coded 
    ...
    numbers may not always be expected or logical. This error form is used for
    higher warning levels, and if the user wants an interupt when upgrading.
    """
...
def ifas_error(type, message):
    """ This is a wrapper around the logging's error command. However, it 
    does not raise an error. Instead, an error may be sent to and it will 
    be converted into a proper warning.

    Parameters
    ----------
    type : ExecptionsClass
        The error that could be raised, but it is more manageable as a logged
        error.
    message : string
    ...
    if (not issubclass(type, Exception)):
        raise AssumptionError("It is assumed that logging errors should be raiseable. This "
                              "type class is not raiseable.")
    try:
...
def ifas_log_warning(type, message):
    """ Just a wrapper function around logging's built-in warn. This is to
    only log a warning but not to display it as a standard warning.
...
class ExportingWarning(Ifas_Warning):
    """
    This warning is used to warn the user about exports and some things that
    may be helpful to know about them. Such exports are generally writing 
    files to disk.
    """

class ErrorWarning(Ifas_Warning):
    """
    This warning is to inform that there was an error that was thrown as a 
    warning (either as a critical warning log or something else.
    """
...
def ifas_info(message, console_print=None):
    ...
    console_print : boolean (optional)
        This ensures that the info message is always printed regardless
        of the logging level set by the logging utility.
...
def ifas_debug(message, console_print=False):
    ...
    console_print : boolean (optional)
        This ensures that the info message is always printed regardless
        of the logging level set by the logging utility. Default is
        False.
...
def ifas_silence_ifas_warnings():
    """ This context manager silences all Ifas based warnings, all other 
...
def ifas_silence_nonifas_warnings():
    """ This context manager silences all non-Ifas based warnings, all 
...
def ifas_absolute_silence():
    ...
    is a wrapper around all other general Ifas context managers (even 
