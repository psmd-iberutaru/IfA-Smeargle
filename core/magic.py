"""
This is to hold functions that are pure 'magic'. That is, functions that 
perform what they are supposed to do, but, are fragile because their
operations are not defined by conventional understanding, but instead as 
...
def magic_inital_gaussian_parameters(x_data, y_data):
    ...
    """
    # This is a magical function, it is not optimal and the user should 
    # know they are using magic. It is best to do more than warn.
    core.error.ifas_error(core.error.MagicError,
                          ("This function (to guess initial Gaussian parameters) is a magic "
                           "function. It is fragile and should only be used if there "
                           "is no better implementation."))
    ...
    fwhm_esti_list = sp_sig.peak_widths(cut_ydata, peak_index, rel_height=0.5)[0]
    try:
        ...
        try:
            ...
            core.error.ifas_warning(core.error.DataWarning,
                                    ("Nyquist peaks cannot be found, relying on 1-bin "
                                     "wide peaks for estimates. Estimates may be very off."))
            peak_index, __ = sp_sig.find_peaks(cut_ydata, width=1, rel_height=1)
            fwhm_esti_list = sp_sig.peak_widths(cut_ydata, peak_index, rel_height=0.5)[0]
            try:
                fwhm_esti = extract_fwhm_estimate(fwhm_esti_list, cut_ydata)
            except ValueError:
                ...
                core.error.ifas_warning(core.error.DataWarning,
                                        ("It seems that there is no peak in the data, "
    ...
    try:
        guess_amplitude = cut_ydata[peak_index[np.argmax(cut_ydata[peak_index])]]
    except ValueError:
        # Just in case the guessing of peaks completely failed, this is an
        # acceptable backup.
